...
const Inventory = (() => {
  ...
  function facts(row,parent){const dl=el('dl',undefined,'detail-list');for(const [key,value] of Object.entries(row)){if(value==null||typeof value==='object'||key.endsWith('_id')||['id','organization_id'].includes(key))continue;dl.append(el('dt',pretty(key)),el('dd',String(value)));}parent.append(dl);}
  ...
  async function open(kind=section,page=1){
    ...
    if(kind==='items'){
      ...
      for(const key of ['total_inventory_items','items_in_stock','low_stock_items','out_of_stock_items','items_requiring_reorder','total_inventory_value'])if(summary[key]!==undefined){const card=el('article');card.append(el('strong',String(summary[key])),el('span',pretty(key)));cards.append(card);}area.append(cards);
    }
  ...
  async function item(id){
    ...
    facts({...data.item,...Object.fromEntries(Object.entries(data).filter(([k,v])=>typeof v!=='object'))},area);
